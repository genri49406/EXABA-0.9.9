...
$delete_img = (int)$_POST['delete_img'];
} else {
$delete_img = 0;
}
...
if ($num_rows == '0') {
$page_title = 'Комментарий для редактирования не найден';
$error = 'Возможно комментарий был удален или вы не опознаны как автор этого коммента и можете его редактировать.';
include '404.php';
exit;
}
...
mysql_query("UPDATE board SET post_title = '$title', post_text = '$text', img_dir = '$img_dir', post_img = '$rnd_name', img_ext = '$img_ext', img_height = '$height', img_width = '$width', thumb_w = '$thumb_w', thumb_h = '$thumb_h', img_size = '$img_size', post_op = '$show_op' WHERE post_id = '$id'");
mysql_close($link);
header('Location: http://'.$_SERVER['SERVER_NAME'].'/'.$web_folder.'post.php?id='.$post_of);
exit;
}

if (empty($rnd_name) && empty($delete_img) && !empty($post_img)) {
mysql_query("UPDATE board SET post_title = '$title', post_text = '$text', post_op = '$show_op' WHERE post_id = '$id'");
}

if (empty($rnd_name) && empty($delete_img) && empty($post_img) && !empty($text)) {
$sql = ("UPDATE board SET post_title = '$title', post_text = '$text', post_op = '$show_op' WHERE post_id = '$id'");
$result = mysql_query($sql, $link);
}

if (!empty($delete_img) && !empty($post_img) && !empty($text)) {
mysql_query("UPDATE board SET post_title = '$title', post_text = '$text', img_dir = '', post_img = '', img_ext = '', img_height = '', img_width = '', thumb_w = '', thumb_h = '', img_size = '', post_op = '$show_op' WHERE post_id = '$id'");
...
header('Location: http://'.$_SERVER['SERVER_NAME'].'/'.$web_folder.'post.php?id='.$post_of.'#c'.$id);
exit;
?>